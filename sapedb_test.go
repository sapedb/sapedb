...
// TestTheSurfaceIsExactlyTheseThirtyNames fails when a name is added to this
...
// package-scope names, plus 6 methods on Client = 30.
func TestTheSurfaceIsExactlyTheseThirtyNames(t *testing.T) {
	...
	wantClientMethods := []string{
		"Welcome", "Operate", "Explore", "WhatIsHere", "Invoke", "Close",
	}

	if got, want := len(wantPackageScope)+len(wantClientMethods), 30; got != want {
		t.Fatalf("this test's own want-lists total %d names, not 30 — the lists drifted from task 0068 §1.5, fix the lists", got)
	}
	...
	if types := methodReceiverTypes(gotMethods); len(types) > 1 || (len(types) == 1 && types[0] != "Client") {
		t.Fatalf("exported methods exist on a type other than Client, which the 30-name surface does not account for: %v", types)
	}
...
func compareNames(t *testing.T, kind string, got, want []string) {
	...
	if len(extra) > 0 {
		t.Errorf("%s(s) exported that are not on the 30-name list: %s", kind, strings.Join(extra, ", "))
	}
	if len(missing) > 0 {
		t.Errorf("%s(s) on the 30-name list that are no longer exported: %s", kind, strings.Join(missing, ", "))
	}
}
