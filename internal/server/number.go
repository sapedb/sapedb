...
//
// It is its own sentinel, and through codeFor its own wire code, rather than
// falling through to "failed" — the shape ISS-21 records. A client that reads
// "failed" is told its request broke and has nowhere to go. What actually
// happened is that one value in it cannot be stored as written, and the thing
// to do about it is send that value differently. Those are two different
// sentences and they need two different codes.
var ErrPrecision = errors.New("sapedb/server: that number cannot be stored as sent")

// Where this check lives, and why it cannot live anywhere later (ISS-35).
//
// `number` is `float64`. A JSON number becomes one at the door, and this file
// is the door for an invoke: from that moment the digits the caller wrote are
// gone. 9007199254740993 and 9007199254740992 are the *same* float64 — the
// same 64 bits, indistinguishable by anything downstream of the decode.
//
// So a check placed after the decode, on the float, can only ever catch SOME
// of the losses, and not the ones worth catching:
//
...
//
// There is no honest version of this check downstream of the decode, so it is
// here, against the literal text the caller sent, which is the last place the
// original value still exists. That is also why invoke() decodes with
// UseNumber: the whole point is to hold the digits back from float64 long
...
// it is the only statement of intent that reaches this server.
//
...
func carries(literal json.Number) (value float64, instead string, ok bool) {
	...
	if err != nil {
		...
		// before this path turned UseNumber on, so nothing newly fails here:
		// what changes is that it is refused under a code of its own instead
		// of as an unreadable request.
		return 0, "", false
	}
...
// survive with the float64 the rest of the server has always been handed.
//
// The replacement matters as much as the check: everything downstream of here
// — store's `matches`, internal/keys, the change log — expects float64 and
// says so, and a json.Number leaking past this point would be a value of a
// type none of them know, which is a second bug wearing the first one's fix as
// a disguise. Nothing downstream changes because nothing downstream sees a
// difference.
func exactArguments(arguments map[string]any) error {
	for name, value := range arguments {
		checked, err := exactly(strconv.Quote(name), value)
		if err != nil {
			return err
		}
...
// exactly is exactArguments applied to one value, naming where it was found.
//
...
func exactly(where string, value any) (any, error) {
	...
	case map[string]any:
		for key, inner := range held {
			checked, err := exactly(where+"."+key, inner)
			if err != nil {
				return nil, err
			}
			held[key] = checked
	...
	case []any:
		for index, inner := range held {
			checked, err := exactly(fmt.Sprintf("%s[%d]", where, index), inner)
			if err != nil {
				return nil, err
			}
			held[index] = checked
